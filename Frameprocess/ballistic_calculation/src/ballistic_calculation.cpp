...
#include"../inlude/ballistic_calculation.hpp"
...
rm_auto_aim::Ballistic::Ballistic()    
{
    YAML::Node config = YAML::LoadFile("/home/pnx/training_code/config.yaml");
    this->bulletV = config["ballistic_calcultion"]["bullet_speed"].as<double>();
    this->K1 = config["ballistic_calcultion"]["iteration_coeffcient_first"].as<double>();
    //第一次大迭代时的步长
    this->K2 = config["ballistic_calcultion"]["iteration_coeffcient_second"].as<double>();
    //第二次大迭代时的步长
    this->k = config["ballistic_calcultion"]["air_resistence"].as<double>(); 
    //空气阻力系数
}
...
std::pair<double , double> Ballistic::fixTiteratPitch(double& horizon_dis , double& height)
{
        ...
        std::cout << "init pitch: " << atan2(target_height, dist_horizon) * 180 / M_PI << std::endl;
...
#ifdef DEBUG_COMPENSATION
            std::cout << "iter: " << i + 1 << " " << delta_height << std::endl;
#endif// DEBUG_COMPENSATION
        }
#ifdef DEBUG_COMPENSATION
        std::cout << "res:" << tmp_pitch * 180 / 3.141592 << std::endl;
